...
public class ScathaTrackerElement extends HudElement {
    ...
    protected void renderElement(DrawContext context) {
        ...
        context.drawTextWithShadow(textRenderer, "§lScatha Tracker", 2, 0, new Color(134, 239, 244).getRGB());
        matrices.pop();

        String[] lines = content.split("\n");
        for (int i = 0; i < lines.length; i++) {
            context.drawTextWithShadow(textRenderer, lines[i], 8, (textRenderer.fontHeight + VERTICAL_PADDING) * (i + 1), Color.WHITE.getRGB());
        }
    }
