    ...
    public static ConfigCategory create(Skyblock21Config defaults, Skyblock21Config config) {
        return ConfigCategory.createBuilder()
                ...
                .option(Option.<Boolean>createBuilder()
                        ...
                        .build())
                .option(Option.<Boolean>createBuilder()
                        .name(literal("Tree progress display"))
                        .description(OptionDescription.of(literal("Displays tree chopping progress on screen")))
                        .binding(defaults.foraging.treeProgress, () -> config.foraging.treeProgress, newValue -> config.foraging.treeProgress = newValue)
                        .controller((opt) -> BooleanControllerBuilder.create(opt)
                                .yesNoFormatter()
                                .coloured(true))
                        .build())
